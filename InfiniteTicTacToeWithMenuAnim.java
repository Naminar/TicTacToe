...
public class InfiniteTicTacToeWithMenuAnim extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        try {
            ...
            InputStream stream = getClass().getResourceAsStream("/img/button.png");
            if (stream == null) {
                System.err.println("Не удалось загрузить изображение кнопки! Проверьте путь.");

                buttonBackgroundImage = null;
            } else {
                buttonBackgroundImage = new Image(stream);
            }
        } catch (Exception e) {
            System.err.println("Ошибка при загрузке изображения кнопки: " + e.getMessage());
            e.printStackTrace();
            buttonBackgroundImage = null;
        }
    ...
    private Node createImageButton(String text, double width, double height, Runnable action) {
        ...
        if (buttonBackgroundImage != null) {
            backgroundView.setImage(buttonBackgroundImage);
            backgroundView.setFitWidth(width);
            backgroundView.setFitHeight(height);

        } else {

            Rectangle fallbackBg = new Rectangle(width, height, Color.GRAY);
            fallbackBg.setStroke(Color.DARKGRAY);
            buttonPane.getChildren().add(fallbackBg);
        }
        ...
        if (buttonBackgroundImage != null) {
            buttonPane.getChildren().addAll(backgroundView, textLabel);
        } else {
            buttonPane.getChildren().add(textLabel);
        }



        final double scaleFactor = 1.1;
        ScaleTransition stIn = new ScaleTransition(Duration.millis(150), backgroundView);
        stIn.setToX(scaleFactor);
        stIn.setToY(scaleFactor);

        ScaleTransition stOut = new ScaleTransition(Duration.millis(150), backgroundView);
        stOut.setToX(1.0);
        stOut.setToY(1.0);

        Color originalTextColor = (Color) textLabel.getTextFill();
        Color hoverTextColor = Color.YELLOW;

        buttonPane.setOnMouseEntered(e -> {
            stIn.playFromStart();
            textLabel.setTextFill(hoverTextColor);
            buttonPane.setCursor(Cursor.HAND);
        });

        buttonPane.setOnMouseExited(e -> {
            stOut.playFromStart();
            textLabel.setTextFill(originalTextColor);
    ...
    private Scene createMainMenuScene() {
        ...
        Node playButton = createImageButton("Играть", 220, 70, () -> {
        ...
        Node stylesButton = createImageButton("Стили", 220, 70, () -> {
            primaryStage.setScene(stylesScene);
            primaryStage.setTitle("Настройка Стилей");
        });

        Node exitButton = createImageButton("Выход", 220, 70, Platform::exit);
    ...
    private void redrawCanvas() {
        if (gc == null || canvas.getWidth() <= 0 || canvas.getHeight() <= 0) return;
